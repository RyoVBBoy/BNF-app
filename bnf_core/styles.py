...
/* ══════════════════════════════════════════════════
   タブ = Twitter/X風の下部固定ナビゲーションバー
   ══════════════════════════════════════════════════ */

/* 一番外側(最上位)のst.tabsだけを下部ナビ化したいので、
   .stApp直下1階層目のtab-listだけを狙う */
.stApp > div > div > div > div > div[data-baseweb="tab-list"] {
  position: fixed;
  bottom: 0; left: 0; right: 0;
  z-index: 1000;
  background: rgba(0,0,0,.92);
  backdrop-filter: blur(12px);
  border-top: 1px solid var(--bnf-bdr);
  border-bottom: none;
  justify-content: space-around;
  padding: 6px 4px calc(6px + env(safe-area-inset-bottom));
  gap: 0 !important;
}
/* PCなど画面が広い場合は中央寄せの幅に揃える(headerと同じ720px) */
@media (min-width: 760px) {
  .stApp > div > div > div > div > div[data-baseweb="tab-list"] {
    max-width: 720px; left: 50%; transform: translateX(-50%);
    border-left: 1px solid var(--bnf-bdr); border-right: 1px solid var(--bnf-bdr);
    border-radius: 16px 16px 0 0;
  }
}

/* ナビ本体が固定されるぶん、コンテンツが隠れないよう下に余白を確保 */
section.main > div { padding-bottom: 86px !important; }

/* 各タブ項目をアイコンボタン風に(縦積み・角丸なし・下線なし) */
...
/* 選択中タブ: Twitterのように文字色がアクセントカラーになり、
   背景にうっすら丸いハイライトが入る */
button[data-baseweb="tab"][aria-selected="true"] {
  color: var(--bnf-acc) !important;
  background: var(--bnf-acc-dim) !important;
}
/* 元々の下線ハイライト・下部ボーダーは下部ナビには不要なので消す */
